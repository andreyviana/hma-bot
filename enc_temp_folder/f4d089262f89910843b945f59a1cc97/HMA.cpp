...
//DWORD BOT_TOKEN = GetEnvironmentVariableA("BOT_TOKEN", NULL, 0);
...
int main()
{
	...
	bot.on_message_create([&bot] (const dpp::message_create_t& event) {
		...
		if (message.rfind(BOT_PREFIX, 0) == 0) {
			std::string command = message.substr(BOT_PREFIX.size());

			if (command == "lista") {
				event.reply("to com preguiça, 23 horas já, Miku out");
			} else if (command == "listas") {
				event.reply("bloody lazy");
			} else if (command == "adicionar") {
				event.reply("add aaaaaaaaaa");
			} else if (command == "remover") {
				event.reply("remove legal");
			}
		}
	});
